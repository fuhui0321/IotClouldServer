...
namespace IotCloudService.IotWebServerWebApi.Filters
{
    public class ApiSecurityFilter : ActionFilterAttribute
    {
        public override void OnActionExecuting(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            ...
            {
                signature = HttpUtility.UrlDecode(request.Headers.GetValues("SignToken").FirstOrDefault());
            }
            ...
            //判断请求头是否包含以下参数
            if (string.IsNullOrEmpty(staffid) || (!int.TryParse(staffid, out id) || string.IsNullOrEmpty(timestamp) || string.IsNullOrEmpty(signature)))
            {
                resultMsg = new ResultMsg();
                resultMsg.StatusCode = (int)StatusCodeEnum.ParameterError;
                resultMsg.Info = StatusCodeEnum.ParameterError.GetEnumText();
            ...
            //double ts = ts2 - ts1;
            //bool falg = ts > int.Parse(WebSettingsConfig.UrlExpireTime) * 60000;
